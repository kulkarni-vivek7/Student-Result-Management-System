...
class Login_window_stud:
    def __init__(self,root):
        ...
        #--------------Background Colors----------------
        left_lbl = Label(self.root,bg="#95c8d8",bd=0)
        left_lbl.place(x=0,y=0,width=600,relheight=1)
        
        right_lbl = Label(self.root,bg="#c7ea46",bd=0)
        ...
        title = Label(login_frame,text="STUDENT LOGIN",font=("times new roman",30,"bold"),bg="white",fg="#b80f0a").place(x=250,y=50)
        
        email = Label(login_frame,text="EMAIL ADDRESS",font=("times new roman",18,"bold"),bg="white",fg="#0080fe").place(x=250,y=150)
        self.txt_email = Entry(login_frame,font=("times new roman",15,"bold"),bg="lightgray")
        self.txt_email.place(x=250,y=180,width=350,height=35)
        
        password = Label(login_frame,text="PASSWORD",font=("times new roman",18,"bold"),bg="white",fg="#0080fe").place(x=250,y=250)
    ...
    def forget_password(self):
        if self.cmb_quest.get() == "Select" or self.txt_answer.get() == "" or self.txt_new_password.get() == "":
            ...
            try:
                ...
                cur = con.cursor()
                cur.execute("select * from student_regis where email=? and question=? and answer=?",(self.txt_email.get(),self.cmb_quest.get(),self.txt_answer.get(),))
                row = cur.fetchone()
                if row == None:
                    messagebox.showerror("Error","Please Select the Correct Security Question / Enter Answer",parent=self.root2)
                else:
                    cur.execute("update student_regis set password=? where email=?",(self.txt_new_password.get(),self.txt_email.get(),))
                    con.commit()
                    con.close()
    ...
    def forget_password_window(self):
        if self.txt_email.get() == "":
            ...
            try:
                con = sqlite3.connect(database="rms.db")
                cur = con.cursor()
                cur.execute("select * from student_regis where email=?",(self.txt_email.get(),))
                row = cur.fetchone()
                if row == None:
    ...
    def register_window(self):
        self.root.destroy()
        os.system("python stud_regis.py")
    ...
    def Login(self):
        if self.txt_email.get() == "" or self.txt_password.get() == "":
            ...
            try:
                con = sqlite3.connect(database="rms.db")
                cur = con.cursor()
                cur.execute("select * from student_regis where email=? and password=?",(self.txt_email.get(),self.txt_password.get(),))
                row = cur.fetchone()
                if row == None:
                    ...
                    self.root.destroy()
                    os.system("python Stud_Dashboard.py")                   
                con.close()
